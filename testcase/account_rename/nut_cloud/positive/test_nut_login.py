...
class TestLoginScenarios:
    ...
    def test_input_username(self, nut_cloud_login_page, login_data):
        with allure.step(f"账号输入: {login_data['username']}"):
            result = nut_cloud_login_page.input_username(login_data['username'])
    ...
    def test_click_return_button(self, nut_cloud_login_page):
        with allure.step("点击返回"):
            result = nut_cloud_login_page.click_return_button()
            assert result.get_cloud_name_text() == '网盘', f"返回网盘主页失败"
    
    @allure.story("用户点击显示密码按钮")
    @allure.title("验证用户点击显示密码按钮后文本是否正常显示为明码")
    def test_click_display_button(self, nut_cloud_login_page, login_data):
        result = nut_cloud_login_page \
    ...
    def test_input_username_return_workflow(self, nut_cloud_login_page, setup):
        with allure.step(f"输入账号，点击返回"):
            result = nut_cloud_login_page.input_username(test_username)
    ...
    def test_input_password_return_workflow(self, nut_cloud_login_page, setup):
        with allure.step(f"输入密码，点击返回"):
            result = nut_cloud_login_page.input_password(test_password)
    ...
    def test_input_username_password_login_success(self, nut_cloud_login_page):
        with allure.step("正确登录"):
            with allure.step("输入账号，输入密码"):
                ...
                assert all(char in ['●', '*', '•'] for char in result.get_input_password_text()), "掩码字符不符合预期"
                result.click_sure()
                assert result.get_toast_page_text() == '成功绑定账号', f"登录失败"
